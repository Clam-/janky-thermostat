...
TABLE_CREATE = "CREATE TABLE setting(target_temp REAL, last_position INT, onoff INT, \
    kp REAL, ki REAL, kd REAL, lower INT, upper INT, pos_margin INT, new_pos INT, speed INT)"
TABLE_SCHED_CREATE = "CREATE TABLE schedule(timestamp TEXT PRIMARY KEY, temp REAL)"

# This MUST be in table create order...
ROW_CREATE = "INSERT INTO setting VALUES(:target_temp, :last_position, :onoff, :kp, :ki, :kd, :lower, :upper, :pos_margin, :new_pos, :speed)"
SETTING_DEFAULTS = {
    "target_temp": 22.0,
    "last_position": 8000,
    "onoff": 1,
...
class MoveThread(threading.Thread):
    def __init__(self, q: queue.Queue, target, pos_margin, lower, upper, speed):
        self.q = q
        self.target = target
        self.moving = 0
        self.offset = 4
        self.settings = { "pos_margin": pos_margin, "lower": lower, "upper": upper, "speed": speed }
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                #print(self.settings)
                if (self.moving == UP or self.moving == STOP) and pos < self.target - self.settings["pos_margin"]:
                    if self.moving == STOP and time.monotonic() - lastmove > 2: 
                        if self.moving != UP: motors.motor2.setSpeed(-self.settings["speed"]) # go up
                        self.moving = UP
                    if self.moving == DOWN: lastmove = time.monotonic()
                elif (self.moving == DOWN or self.moving == STOP) and pos > self.target + self.settings["pos_margin"]:
                    if self.moving == STOP and time.monotonic() - lastmove > 2: 
                        if self.moving != DOWN: motors.motor2.setSpeed(self.settings["speed"]) # go down
                        self.moving = DOWN
                    if self.moving == DOWN: lastmove = time.monotonic()
                else: # also stop
...
class Controller:
    def __init__(self, stats):
        ...
        self.q = queue.Queue()
        self.mover = MoveThread(self.q, self.settings.last_position, self.settings.pos_margin, 
            self.settings.lower, self.settings.upper, self.settings.speed)
        self.mover.start()
        self.currentsched = ""
        ...
        try:
            while True:
                ...
                if self.settings.new_pos != 0:
                    ...
                    self.q.put(["P", self.settings.new_pos])
                ...
                if (currentschedcheck - lastschedcheck > 60):
                    # also update actuator settings
                    self.q.put(["S", {"lower": self.settings.lower, "pos_margin": self.settings.pos_margin, 
                        "lower": self.settings.lower, "upper": self.settings.upper, "speed": self.settings.speed }])
...
if __name__ == '__main__':
    ...
    control = Controller(Stats())
    control.loop()
>>>>>>> 38c6c73f07e3a41e598108ae91dd027491376b5c
